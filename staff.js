...
$('#legend').innerHTML = state.stylists.map((b) =>
`<span class="lg"><span class="sw" style="background:${b.color}"></span>${b.name}</span>`).join('') +
`<span class="lg"><span class="sw" style="background:#2a2a2f"></span>Blocked / holiday</span>`;
...
openAppt = async function (id, forDate) {
  ...
  if (!appt || appt.stylist_id !== myStaffId()) return;
