...
col += `<div class="wk-chip wk-block">${bl.reason}${bl.stylist_name ? ' · ' + bl.stylist_name : ''}</div>`;
});
...
<b>${fmt(min)}</b> ${a.client_name}<span class="wk-who">${a.stylist_name} · ${a.service_name}${label}</span></div>`;
});
col += `</div>`;
html += col;
});
...
if (e <= s) return;
col += `<div class="blk" data-id="${bl.id}" style="top:${(s - DAY_START) * PXMIN}px;height:${(e - s) * PXMIN - 2}px" title="Click to view/remove this block">${bl.reason}</div>`;
});
// appointments
